...
spring.rabbitmq.listener.direct.missing-queues-fatal: false #若容器声明的队列在代理上不可用，是否失败.
...
spring.rabbitmq.listener.direct.retry.multiplier: 1 # 应用于前一个重试间隔的倍增器。
spring.rabbitmq.listener.direct.retry.stateless: true # 重试是无状态还是有状态的。
```
